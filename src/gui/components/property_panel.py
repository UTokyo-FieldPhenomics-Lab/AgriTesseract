...
class SubplotPropertyPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        numbering_group = PropertyGroup("编号规则")
        ...
        rotation_group.add_row("旋转角度:", self.spin_rotation)

        self.check_auto_rotate = QCheckBox("自动计算")
...
class SeedlingPropertyPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        detect_group = PropertyGroup("检测设置")
        ...
        detect_group.add_row("置信度阈值:", self.spin_confidence)

        self.spin_iou = QDoubleSpinBox()
        self.spin_iou.setRange(0.1, 0.9)
        self.spin_iou.setValue(0.4)
        self.spin_iou.setSingleStep(0.05)
        detect_group.add_row("NMS IoU:", self.spin_iou)

        layout.addWidget(detect_group)

        # Statistics Group
        stats_group = PropertyGroup("统计信息")

        self.label_total = QLabel("0")
        stats_group.add_row("检测总数:", self.label_total)

        self.label_selected = QLabel("0")
        stats_group.add_row("已选择:", self.label_selected)
...
class TimeSeriesPropertyPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        # Data Sources Group
        data_group = PropertyGroup("数据源")

        self.label_date_count = QLabel("0 个日期")
        data_group.add_row("已添加:", self.label_date_count)

        self.label_point_count = QLabel("0 个点")
        data_group.add_row("苗位置:", self.label_point_count)

        layout.addWidget(data_group)

        # Output Options Group
        output_group = PropertyGroup("输出选项")

        self.combo_format = QComboBox()
        self.combo_format.addItems(["PNG", "JPG", "TIFF"])
        output_group.add_row("图片格式:", self.combo_format)

        self.check_create_subfolders = QCheckBox()
        self.check_create_subfolders.setChecked(True)
        output_group.add_row("按株分目录:", self.check_create_subfolders)
...
class AnnotatePropertyPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        adjust_group = PropertyGroup("图像调节")
        ...
        adjust_group.add_row("亮度:", self.spin_brightness)
        ...
        adjust_group.add_row("对比度:", self.spin_contrast)
        ...
        adjust_group.add_row("Gamma:", self.spin_gamma)

        layout.addWidget(adjust_group)

        # Instance List Group
        instance_group = PropertyGroup("标注实例")

        self.label_instance_count = QLabel("0 个实例")
        instance_group.add_row("当前图片:", self.label_instance_count)

        layout.addWidget(instance_group)

        # Class Management Group
        class_group = PropertyGroup("类别管理")

        self.combo_current_class = QComboBox()
        self.combo_current_class.addItems(["leaf", "stem", "background"])
        class_group.add_row("当前类别:", self.combo_current_class)
        ...
        train_group = PropertyGroup("训练状态")

        self.label_annotated_count = QLabel("0 / 0")
        train_group.add_row("已标注图片:", self.label_annotated_count)

        self.label_model_status = QLabel("未训练")
        train_group.add_row("模型状态:", self.label_model_status)
