"""Subplot generation tab based on GeoPandas."""
...
class SubplotTab(TabInterface):
    """UI workflow for subplot generation using GeoPandas."""
    ...
    def _build_layout_tab(self) -> QWidget:
        ...
        self.combo_def_mode.addItems(
            [tr("page.subplot.combo.rc"), tr("page.subplot.combo.size")]
        )
        ...
        layout_rc.addWidget(
            self._build_labeled_widget(tr("page.subplot.label.rows"), self.spin_rows)
        )
        layout_rc.addWidget(
            self._build_labeled_widget(tr("page.subplot.label.cols"), self.spin_cols)
        )
        ...
        layout_size.addWidget(
            self._build_labeled_widget(
                tr("page.subplot.label.width_m"), self.spin_width
            )
        )
        layout_size.addWidget(
            self._build_labeled_widget(
                tr("page.subplot.label.height_m"), self.spin_height
            )
        )
        ...
        bar.addWidget(
            self._build_labeled_widget(
                tr("page.subplot.label.def_mode"), self.combo_def_mode
            )
        )
        bar.addWidget(self.stack_layout_mode)
        bar.addWidget(
            self._build_labeled_widget(
                tr("page.subplot.label.x_space"), self.spin_x_spacing
            )
        )
        bar.addWidget(
            self._build_labeled_widget(
                tr("page.subplot.label.y_space"), self.spin_y_spacing
            )
        )
        bar.addWidget(
            self._build_labeled_widget(tr("page.subplot.label.keep"), self.combo_keep)
        )
        bar.addSeparator()
    ...
    def _build_numbering_tab(self) -> QWidget:
        ...
        bar.addWidget(
            self._build_labeled_widget(
                tr("prop.label.numbering_mode"), self.combo_numbering_mode
            )
        )
        bar.addWidget(
            self._build_labeled_widget(tr("prop.label.start_row"), self.spin_start_row)
        )
        bar.addWidget(
            self._build_labeled_widget(tr("prop.label.start_col"), self.spin_start_col)
        )
        bar.addWidget(
            self._build_labeled_widget(tr("prop.label.prefix"), self.edit_prefix)
        )
        bar.addWidget(
            self._build_labeled_widget(tr("prop.label.suffix"), self.edit_suffix)
        )
        bar.addWidget(self._bar_spacer())
        layout.addWidget(bar)
        return tab
    ...
    def _show_preview(self) -> None:
        ...
        preview_gdf = generate_subplots_gdf(self.boundary_gdf, **params)
        self.last_preview_gdf = preview_gdf
    ...
    def _on_load_image(self) -> None:
        ...
        InfoBar.error(
            title=tr("error"), content=f"Failed to load image: {file_path}", parent=self
        )

    @Slot()
    def _on_load_boundary(self) -> None:
        """Load boundary shapefile into GeoDataFrame model."""
        file_path, _ = QFileDialog.getOpenFileName(
            self,
        ...
        try:
            self.boundary_gdf = load_boundary_gdf(file_path)
            self.map_component.map_canvas.add_vector_layer(
                self.boundary_gdf,
                "Boundary",
                color="#FF0000",
    ...
    def _on_focus(self) -> None:
        ...
        if self.boundary_gdf is not None:
            ...
            # Auto-rotate
            angle = calculate_optimal_rotation(self.boundary_gdf)
    ...
    def _on_generate(self) -> None:
        ...
        try:
            ...
            InfoBar.error(
                title=tr("error"), content=f"Generation failed: {exc}", parent=self
            )
