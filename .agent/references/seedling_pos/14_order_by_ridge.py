...
bound_path = NAS_ROOT / "02_GIS" / "boundary.shp"

SHP_DIR = NAS_ROOT / "11_root_position" / "shapefiles"

# ==============================
center_points_path = SHP_DIR / "1_edited_center_points.shp"
...
#                             shp_proj=proj)
process_area = gpd.read_file(bound_path)
...
center_points_np = center_points.get_coordinates().values
center_points_px = dom.geo2pixel(center_points_np)
...
bound_np = process_area.get_coordinates().values
process_area_px = dom.geo2pixel(bound_np)
...
selection_onoff = True
if ignored_num == 0:
    save_shp(order_df, SHP_DIR / "ordered_center_points", type='points')
    selection_onoff = False
...
while selection_onoff:
    ...
    if ans == '1':
        ...
        selection_onoff = False
        save_shp(fid_positive, SHP_DIR / "ordered_center_points", type='points')
    else:
